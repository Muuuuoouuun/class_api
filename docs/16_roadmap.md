...
- [x] `local_data/inbox` 폴더 규칙 정의: 오프라인 출결 CSV, 성적 XLSX, 상담 메모, 첨부 자료
- [x] 로컬/오프라인 공유 데이터 정규화 + 보수적 학생 매칭
- [x] 매칭 실패/중복 후보를 선생님 상황판의 `확인 필요` 큐로 노출
- [x] 병합된 context를 주간 리포트와 AI 질문 응답에 재사용

## Academy Ops Hub 제품 축

- [x] ClassIn API Push / Data Subscription / 학원 데이터 융합 / 개별 리포트 4축 스펙 정의
- [x] `/api/ops-hub`로 4축 상태와 오늘 처리할 학생 큐 통합
- [x] `/api/ops-hub`가 4축 상태를 실행 순서 브리핑으로 변환
- [x] 개별 리포트 프롬프트에 융합 데이터 출처를 더 구조적으로 주입
- [x] 원장/교사용 agent에 `query_academy_context` 도구 추가
- [x] 숙제 미제출 알림 문구에 품질 게이트 추가
- [x] 주간 리포트 드래프트에 AI 품질 점검 상태와 경고 노출
- [x] `blocked` 품질 리포트가 기본 승인/아카이브로 넘어가지 않도록 게이트 추가
- [x] 허브 UI에서 주간 드래프트 품질 큐를 조회하고 승인 전 경고를 확인
- [x] 파일럿 브링업 브리프와 Windows 작업 스케줄러 등록 스크립트 제공
- [ ] 원장용 데모 영상에서 4축 흐름을 3~5분 플롯으로 재촬영
