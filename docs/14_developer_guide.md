...
도구 추가는 [skills.py](../src/classin_toolkit/intelligence/skills.py) 의 `TOOLS` 리스트 + `execute_tool` 분기에 한 곳에서만 한다.
...
UI는 기존 파이프라인을 감싸는 얇은 FastAPI 화면이다. 상태/API 점검, 미제출 조회와
선택 발송, 스케줄표 dry-run/생성, 일일 HTML 생성, 주간 드래프트 생성/승인,
시험 CSV dry-run/import, 메모 작성, 단발 AI 질문을 실행한다.
...
### 4.2 새 CED action 추가
1. [ced.py](../src/classin_toolkit/classin/ced.py)에 메서드 추가. v1 action 은 `self._c.call_v1("actionName", body)`, v2 LMS 경로는 `self._c.call_v2("/lms/...", body)` 호출. 반환 `data` 에서 ID 추출.
2. 필요 시 [schemas.py](../src/classin_toolkit/classin/schemas.py) 도메인 모델 확장.
3. pipelines에서 새 메서드 활용.

스케줄 생성 흐름을 바꿀 때는 [core_engine.py](../src/classin_toolkit/pipelines/core_engine.py)의 `classin.schedule_api` 분기와 `classin.teacher_uids` 매핑도 같이 확인한다. LMS classroom 생성은 `teacherUid` 가 필수다.
...
### 4.4 새 에이전트 도구 추가
1. [skills.py](../src/classin_toolkit/intelligence/skills.py) `TOOLS` 리스트에 Anthropic tool 스키마 추가.
2. `execute_tool` 에 분기 추가. Notion/파이프라인 호출.
